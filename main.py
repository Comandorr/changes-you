...
text_center = SimpleText('CHANGES', 64, win_w/2, win_h/2)
text_center.position[0] = win_w/2 - text_down.rect.width/2
...
while run:
    ...
    if ground[-1].x <= world_x + win_w/2:
        ...
        for y in range(win_h//64+1):
            ...
            ground.append(scene_car[-1])
            if chance(5):
                scene_car.append(SimpleSprite('images/crate_12.png', x+64, y*64, size = (32, 32)))
